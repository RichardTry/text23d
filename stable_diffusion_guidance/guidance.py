...
class StableDiffusionGuidance():
    ...
    def calculate_loss(self, generated_render, batch_size, H, W, concrete_directions, guidance_scale=100, grad_scale=1):
        ...
        t = torch.randint(self.min_step, self.max_step + 1, (latents.shape[0],), dtype=torch.long, device=self.torch_device)
    ...
    def create_text_embedding_dict(self, prompt):
        for direction in self.directions:
            ...
            text_input = self.tokenizer(extended_prompt, padding="max_length", max_length=self.tokenizer.model_max_length, truncation=True, return_tensors="pt")
            with torch.no_grad():
                self.text_embeddings[direction] = self.text_encoder(text_input.input_ids.to(self.torch_device))[0]
        ...
        with torch.no_grad():
            self.text_embeddings['unconditional'] = self.text_encoder(uncond_input.input_ids.to(self.torch_device))[0]
    ...
    def map_directions_to_text_embeddings(self, directions):
        ...
        uncond_embeddings = [self.text_embeddings['unconditional'] for _ in range(len(directions))]
