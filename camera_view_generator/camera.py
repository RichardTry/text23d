...
sys.path.append("..")
from tiny_nerf.utils.ray_utils import get_ray_directions, get_rays  # noqa: E402
...
class Directions(Enum):
    ...
    SIDE_RIGHT = (
        ...
        "side",
    )  # phi \in [180+front/2, 360-front/2)
...
def get_directions_from_spherical(thetas, phis, overhead_angle, front_angle):
    ...
    # phis = phis % (2 * np.pi)
    dirs[
        (phis < front_angle / 2) | (phis >= 2 * np.pi - front_angle / 2)
    ] = Directions.FRONT
    dirs[
        (phis >= front_angle / 2) & (phis < np.pi - front_angle / 2)
    ] = Directions.SIDE_LEFT
    dirs[
        (phis >= np.pi - front_angle / 2) & (phis < np.pi + front_angle / 2)
    ] = Directions.BACK
    dirs[
        (phis >= np.pi + front_angle / 2) & (phis < 2 * np.pi - front_angle / 2)
...
class CameraDataset(Dataset):
    def __init__(self, label="default", H=64, W=64, samples=0):
        ...
        self.allrays, self.directions = generate_all_rays(
            self.samples, self.H, self.W, self.focal
        )
    ...
    def __getitem__(self, idx):
        return {"rays": self.allrays[idx], "dirs": int(self.directions[idx])}
